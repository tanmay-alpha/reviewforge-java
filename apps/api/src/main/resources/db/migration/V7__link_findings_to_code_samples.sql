-- ============================================
-- V7: link findings to persisted code samples
-- ============================================
-- Adds an optional FK from findings to ml.code_samples so
-- every finding produced during a PR scan can be traced back
-- to the exact redacted hunk that triggered it.

ALTER TABLE findings
    ADD COLUMN IF NOT EXISTS code_sample_id UUID;

ALTER TABLE findings
    ADD CONSTRAINT fk_findings_code_sample
